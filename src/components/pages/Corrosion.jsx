    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            {MECHANISM_LABELS[data.dominantMechanism] ||
              data.dominantMechanism ||
              "—"}
          </div>
        </div>
      </div>
      ...
      {worst && (
        ...
        >
          ...
          >
            {worst.remainingLifeYrs != null
              ? `${worst.remainingLifeYrs}y remaining`
              : "—"}
          </span>
        </div>
      )}
      ...
      >
        ...
        >
          {Array(4)
            .fill(0)
            .map((_, i) => (
              <Skeleton key={i} w="100%" h={80} r={10} t={t} />
            ))}
        </div>
      ...
      <div style={{ flex: 1, padding: "20px 24px", overflow: "auto" }}>
        ...
        {activeTab === "Overview" && (
          ...
          >
            ...
            >
              {[
                {
                  label: "Total Corrosion Rate",
                  value: `${detail.corrosionRate?.total?.value?.toFixed(3) ?? "—"} mm/yr`,
                  color,
                },
                {
                  label: "Operating Temp",
                  value: `${detail.conditions?.temperature?.value ?? liveUnit?.temp ?? "—"} °C`,
                  color: t.amber,
                },
                {
                  label: "Operating Press",
                  value: `${detail.conditions?.pressure?.value ?? liveUnit?.pressure ?? "—"} bara`,
                  color: t.blue,
                },
                {
                  label: "Stream pH",
                  value: detail.streamChemistry?.pH?.value?.toFixed(2) ?? "—",
                  color: t.teal,
                },
                ...
                {[
                  {
                    ...
                    value: `${detail.streamChemistry?.h2sConc?.value?.toFixed(1) ?? "—"} mg/L`,
                  },
                  {
                    label: "CO₂ Partial Press",
                    value: `${detail.streamChemistry?.co2PartialPress?.value?.toFixed(3) ?? "—"} bar`,
                  },
                  {
                    label: "Dominant Mechanism",
                    value:
                      MECHANISM_LABELS[detail.dominantMechanism] ||
                      detail.dominantMechanism ||
                      "—",
                  },
                ].map(({ label, value }) => (
                  <div
        ...
        {activeTab === "Mechanisms" && (
          ...
          >
                    ...
                    <Bar dataKey="rate" name="mm/year" radius={[0, 4, 4, 0]}>
                      {mechData.map((e, i) => (
                        <Cell
                          key={i}
                          fill={
                            e.key === detail.dominantMechanism
                              ? color
                              : t.accent
                          }
                          fillOpacity={
                            e.key === detail.dominantMechanism ? 0.9 : 0.5
                          }
                        />
                      ))}
                    </Bar>
                ...
                >
                  {mechData.map((m) => {
                    const pctOfTotal = detail.corrosionRate?.total?.value
                      ? parseFloat(
                          (
                            (m.rate / detail.corrosionRate.total.value) *
                            100
                          ).toFixed(1),
                        )
                      : 0;
                        ...
                        >
                          <div
                            style={{
                              ...
                              width: `${pctOfTotal}%`,
                              borderRadius: "2px",
                              background: isDominant ? color : t.accent,
                              opacity: 0.7,
                            }}
                          />
                        </div>
            ...
            >
              <strong style={{ color: t.textPrimary }}>OLI Engine Note:</strong>{" "}
              Rates computed using first-principles electrolyte thermodynamics
              (MSE framework). Sulfidic rates follow McConomy/API 939-C curves.
              Naphthenic acid follows Couper-Gorman/API 581. CO₂ rates follow de
              Waard-Milliams model.
              {detail.mode === "mock" &&
                " Currently running in mock mode — connect OLI credentials for live calculation."}
            </div>
          </div>
        )}
            ...
            >
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns:
                    "1fr 1.5fr 0.7fr 0.7fr 0.7fr 0.8fr 0.8fr 0.8fr",
                  padding: "8px 16px",
                  background: t.bgSubtle,
                  borderBottom: `1px solid ${t.border}`,
                }}
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        borderRadius: "4px",
                        width: "fit-content",
                        background: `${ptColor}14`,
                        border: `1px solid ${ptColor}28`,
                      }}
                    >
                      <span
                        style={{
          ...
          >
            ...
            >
              {Object.entries(detail.riskFlags || {}).map(([flag, value]) => {
                ...
                const descriptions = {
                  hthaRisk:
                    "Risk per API 941 Nelson curves. High H₂ partial pressure + temperature can cause irreversible steel embrittlement.",
                  polythionicAcidSCC:
                    "Risk during shutdowns when polythionic acid forms on catalytic surfaces. Use austenitic SS or neutralize with soda ash.",
                  stressCorrCracking:
                    "Wet H₂S service. Risk of hydrogen-induced cracking (HIC) and stress-oriented HIC (SOHIC) in weld HAZ.",
                  thermalFatigue:
                    "Repeated thermal cycling of coke drums causes weld cracking and shell distortion. Monitor bulging and skirt cracking.",
                };
                const isActive =
                  value === true || value === "high" || value === "elevated";
                const flagColor =
                  value === "high"
                    ? t.bad
                    : value === "elevated" || isActive
                      ? t.warn
                      : t.good;
                const FlagIcon =
                  value === "high" || isActive ? AlertTriangle : CheckCircle;
                    ...
                    >
                      <div
                        style={{
                          padding: "2px 8px",
                          borderRadius: "4px",
                          background: `${flagColor}14`,
                          border: `1px solid ${flagColor}28`,
                        }}
                      >
                        <span
                          style={{
            ...
            >
              ...
              API 510 (pressure vessels) · API 570 (piping) · API 579 (fitness
              for service) · API 941 (HTHA) · API 939-C (sulfidic corrosion) ·
              API 581 (RBI)
            </div>
          </div>
        )}
...
export default function Corrosion({
  ...
  const maxRate = allData.length
    ? Math.max(
        ...allData.map(
          (d) => d.totalRateMmYr ?? d.corrosionRate?.total?.value ?? 0,
        ),
      )
    : 0;
    ...
    <div style={{ padding: "24px", height: "100%", overflow: "auto" }}>
      ...
      >
        <div>
          ...
          >
            OLI Systems Engine API ·{" "}
            {allData[0]?.mode === "mock" ? "Mock Mode" : "Live"} · 7 units
            monitored · Updates every 90s
          </div>
        </div>
        <button
        ...
        {[
          {
            ...
            value: critical,
            color: critical > 0 ? t.bad : t.good,
          },
          {
            label: "Warning Units",
            value: warning,
            color: warning > 0 ? t.warn : t.good,
          },
          {
            label: "Max Rate (mm/yr)",
            value: maxRate.toFixed(3),
            color: maxRate > 0.5 ? t.warn : t.good,
          },
          {
      ...
      {allData[0]?.mode === "mock" && (
        ...
        >
          ...
          >
            <strong style={{ color: t.accent }}>Mock mode active.</strong> Rates
            computed by built-in empirical models (McConomy, Couper-Gorman, de
            Waard-Milliams). To enable live OLI Cloud: set{" "}
            <code
              style={{
                background: t.bgSubtle,
                padding: "1px 5px",
